...
(Besides warning everyone of the Coming IPv4 Apocalypse Of Doom And Gloom, [Vint Cerf](https://en.wikipedia.org/wiki/Vint_Cerf)[14](https://beej.us/guide/bgnet/html/split-wide/more-references.html#fn14) is also well-known for being The Father Of The Internet. So I really am in no position to second-guess his judgment.)
...
If you’ll kindly remember, I presented you earlier with the [Layered Network Model](https://beej.us/guide/bgnet/html/split-wide/what-is-a-socket.html#lowlevel) which had the Internet Layer (IP) split off from the Host-to-Host Transport Layer (TCP and UDP). Get up to speed on that before the next paragraph.
...
Well, different services on the Internet have different well-known port numbers. You can see them all in [the Big IANA Port List](https://www.iana.org/assignments/port-numbers)[16](https://beej.us/guide/bgnet/html/split-wide/more-references.html#fn16) or, if you’re on a Unix box, in your `/etc/services` file. HTTP (the web) is port 80, telnet is port 23, SMTP is port 25, the game [DOOM](https://en.wikipedia.org/wiki/Doom_%281993_video_game%29)[17](https://beej.us/guide/bgnet/html/split-wide/more-references.html#fn17) used port 666, etc. and so on. Ports under 1024 are often considered special, and usually require special OS privileges to use.
...
The thing is, everyone in the Internet world has generally agreed that if you want to represent the two-byte hex number, say `b34f`, you’ll store it in two sequential bytes `b3` followed by `4f`. Makes sense, and, as [Wilford Brimley](https://en.wikipedia.org/wiki/Wilford_Brimley)[18](https://beej.us/guide/bgnet/html/split-wide/more-references.html#fn18) would tell you, it’s the Right Thing To Do. This number, stored with the big end first, is called _Big-Endian_.
...
There are no standard 64-bit variants in the sockets API, but I talk about other options in the [`htons()` reference page](https://beej.us/guide/bgnet/html/split-wide/man-pages.html#htonsman). And if you want to do floating point, check out the section on [Serialization](https://beej.us/guide/bgnet/html/split-wide/slightly-advanced-techniques.html#serialization), far below.
